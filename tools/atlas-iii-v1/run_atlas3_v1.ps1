...
$Base = "https://raw.githubusercontent.com/yum4nity-code/guar/main/tools/atlas-iii-v1"

$Files = @(
    @{ Name = "atlas3_engine_v1.py"; Blob = "fd888ae28bdc21cb19ffe705558dca0a2394f7a0" },
    @{ Name = "atlas3_analyze_v1.py"; Blob = "dd24cdc6aea8bed975c690004a1e156c6e592e48" }
)
...
$Tmp = Join-Path $env:TEMP ("atlas3_v1_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $Engine = Join-Path $Campaign "atlas3_engine_v1.py"
    $Analyzer = Join-Path $Campaign "atlas3_analyze_v1.py"
    $RunId = "ATLAS-III-V1-" + (Get-Date -Format "yyyyMMdd-HHmmss")
    ...
    Write-Host ""
    Write-Host "===== ATLAS III PREFLIGHT ====="
    Write-Host "One market pass planned. Discovery 2017-2022 only."
    Write-Host "2023-2025 CLOSED. 2026 CLOSED."
    Write-Host "Two AMPLITUDE families can only pass a phenomenon gate, never a tradable gate."

    & python $Engine --manifest $Manifest --standards $Standards --output-dir $Run
    if ($LASTEXITCODE -ne 0) {
        throw "Atlas III preflight failed"
    }

    Write-Host ""
    Write-Host "===== ATLAS III MARKET PASS ====="
    Write-Host "Do not enter console selection mode while this is running. If output appears frozen after selecting text, press Esc."

    & python $Engine --manifest $Manifest --standards $Standards --output-dir $Run --execute
    if ($LASTEXITCODE -ne 0) {
        throw "Atlas III engine failed. Keep the run directory for diagnosis."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Atlas III analyzer failed"
    }

    Write-Host ""
    Write-Host "===== ATLAS III COMPLETE ====="
    Write-Host "RUN: $Run"
    Write-Host "REPORT: $(Join-Path $Analysis 'atlas3_analysis.json')"
