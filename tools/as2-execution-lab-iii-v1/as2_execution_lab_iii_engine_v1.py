...
CANDIDATES = [
    {"id": "BASELINE_TIME_H60", "kind": "TIME", "time": 60},
    {"id": "SL_ONLY_1p5R_H60", "kind": "STOP_ONLY", "sl": 1.5, "time": 60},
    {"id": "SL_ONLY_2R_H60", "kind": "STOP_ONLY", "sl": 2.0, "time": 60},
    {"id": "SL_ONLY_2p5R_H60", "kind": "STOP_ONLY", "sl": 2.5, "time": 60},
    {"id": "SL_ONLY_3R_H60", "kind": "STOP_ONLY", "sl": 3.0, "time": 60},
    {"id": "BE_AFTER_1p5R_SL2p5R_H60", "kind": "BE_NO_TP", "be": 1.5, "sl": 2.5, "time": 60},
    {"id": "BE_AFTER_2R_SL2p5R_H60", "kind": "BE_NO_TP", "be": 2.0, "sl": 2.5, "time": 60},
]
...
FIELDNAMES = [
    ...
    "be_activated", "crossed_calendar_gap",
]
...
def admitted_rows(dl, pf, manifest_path: Path):
    ...
    if (
        ...
        raise dl.AdmissionError("Execution Lab III requires XAUUSD UTC M1")
...
class Lab:
    ...
    def step_stop_only(self, bar, p):
        c = p.candidate
        _, adv = self.excursions(bar, p)
    ...
    def step_be_no_tp(self, bar, p):
        ...
        if p.be_active:
            ...
            be_trigger = fav >= c["be"] * p.risk

            # Conservative unknown intrabar order: if the initial stop and
            # the favorable BE trigger are both present in the same M1 bar,
            # score the initial stop first.
            if sl_hit:
                reason = (
                    "AMBIGUOUS_INITIAL_SL_FIRST"
                    if be_trigger
                    else "SL_BEFORE_BE"
                )
                self.write_exit(p, -c["sl"], reason)
                return True

            if be_trigger:
                # BE protection starts only on the next M1 bar.
                p.be_active = True
    ...
    def process_positions(self, bar):
        ...
        for p in self.positions:
            ...
            if kind == "TIME":
                ...
                done = self.step_stop_only(bar, p)
            elif kind == "BE_NO_TP":
                done = self.step_be_no_tp(bar, p)
            else:
...
def main():
    ...
    preflight = {
        ...
        "entry_rule": (
            "AS2_SKEW_VOV frozen: hourly abs(skew)>=1.0 and "
            "VOV CV>=0.75; signal trades opposite skew"
        ),
    ...
    spec = {
        ...
        "intrabar_policy": {
            "stop_only": "SL is evaluated on every M1; no profit cap",
            "be_trigger": "BE activates only from next M1",
            "be_trigger_and_initial_sl_same_m1": "initial SL first",
            "after_be": "first M1 touching entry exits at 0R",
        },
        "cost_semantics": (
            "subtract absolute XAUUSD price cost 0.10/0.20 "
            "divided by row risk from gross R"
        ),
            ...
            for bar in dl.decode_dukascopy_m1_day(row):
                if not (
                    ...
                    raise dl.AdmissionError(
                        "bar outside Execution Lab III hard wall"
                    )
                lab.on_m1(bar)
