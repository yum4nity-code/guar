...
$Base = "https://raw.githubusercontent.com/yum4nity-code/guar/main/tools/as2-execution-lab-iii-v1"

$Files = @(
    @{ Name = "as2_execution_lab_iii_engine_v1.py"; Blob = "11f0c84f9c364890fd327372ee1964437b431c1f" },
    @{ Name = "as2_execution_lab_iii_analyze_v1.py"; Blob = "fed5e17a3b71d21e1ecee00dc2e4b4abe723261d" }
)
...
$Tmp = Join-Path $env:TEMP ("as2_exec_lab_iii_v1_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $Engine = Join-Path $Campaign "as2_execution_lab_iii_engine_v1.py"
    $Analyzer = Join-Path $Campaign "as2_execution_lab_iii_analyze_v1.py"

    $RunId = "AS2-EXECUTION-LAB-III-V1-" + (Get-Date -Format "yyyyMMdd-HHmmss")
    ...
    Write-Host ""
    Write-Host "===== AS2 EXECUTION LAB III PREFLIGHT ====="
    Write-Host "ENTRY: AS2_SKEW_VOV frozen."
    Write-Host "HOLD: H60 only."
    Write-Host "PROFIT CAP: NONE."
    Write-Host "TEST: downside-only stops and BE protection."
    Write-Host "DEVELOPMENT: 2017-2025, already contaminated."
    Write-Host "2026: CLOSED."
    Write-Host "7 preregistered candidates."
    Write-Host "Analyzer must reproduce Lab II TIME_H60 exactly before accepting results."

    & python $Engine --manifest $Manifest --standards $Standards --output-dir $Run
    if ($LASTEXITCODE -ne 0) {
        throw "AS2 Execution Lab III preflight failed"
    }

    Write-Host ""
    Write-Host "===== AS2 EXECUTION LAB III MARKET PASS ====="
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "AS2 Execution Lab III engine failed. Keep the run directory for diagnosis."
    }
    ...
    Write-Host ""
    Write-Host "===== AS2 EXECUTION LAB III ANALYSIS ====="
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "AS2 Execution Lab III analyzer failed"
    }

    Write-Host ""
    Write-Host "===== AS2 EXECUTION LAB III COMPLETE ====="
    Write-Host "RUN: $Run"
    Write-Host "REPORT: $(Join-Path $Analysis 'as2_execution_lab_iii_analysis.json')"
