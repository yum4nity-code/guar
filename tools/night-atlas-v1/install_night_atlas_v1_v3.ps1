...
$Tmp = Join-Path $env:TEMP ("night_atlas_v1_v3_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    foreach ($part in $parts) {
        Write-Host "DOWNLOAD $part"
        Invoke-WebRequest "$Base/chunks/$part" -OutFile (Join-Path $Tmp $part)
    }
    ...
    try {
        foreach ($part in $parts) {
            $bytes = [IO.File]::ReadAllBytes((Join-Path $Tmp $part))
    ...
    foreach ($name in @(
        "night_atlas_analyze_v1.py",
        "night_atlas_cross_scan_v1.py",
        "run_night_atlas_v1.ps1"
    )) {
        Write-Host "DOWNLOAD $name"
        Invoke-WebRequest "$Base/$name" -OutFile (Join-Path $Tmp $name)
    }
