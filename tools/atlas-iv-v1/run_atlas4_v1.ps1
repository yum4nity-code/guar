...
$Base = "https://raw.githubusercontent.com/yum4nity-code/guar/main/tools/atlas-iv-v1"

$Files = @(
    @{ Name = "atlas4_engine_v1.py"; Blob = "413c0203862f5a39cbafe55e1620d243c4d7187a" },
    @{ Name = "atlas4_analyze_v1.py"; Blob = "f23e29d8253075c9149cafc0f4359cfb28cade3f" }
)
...
$Tmp = Join-Path $env:TEMP ("atlas4_v1_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $Engine = Join-Path $Campaign "atlas4_engine_v1.py"
    $Analyzer = Join-Path $Campaign "atlas4_analyze_v1.py"
    $RunId = "ATLAS-IV-V1-" + (Get-Date -Format "yyyyMMdd-HHmmss")
    ...
    Write-Host ""
    Write-Host "===== ATLAS IV PREFLIGHT ====="
    Write-Host "One market pass planned. Discovery 2017-2022 only."
    Write-Host "2023-2025 CLOSED. 2026 CLOSED."
    Write-Host "Independent families only; no A3_02/A3_06 tuning."
    Write-Host "Two AMPLITUDE families can pass only a phenomenon gate."

    & python $Engine --manifest $Manifest --standards $Standards --output-dir $Run
    if ($LASTEXITCODE -ne 0) { throw "Atlas IV preflight failed" }

    Write-Host ""
    Write-Host "===== ATLAS IV MARKET PASS ====="
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Atlas IV engine failed. Keep the run directory for diagnosis."
    }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Atlas IV analyzer failed" }

    Write-Host ""
    Write-Host "===== ATLAS IV COMPLETE ====="
    Write-Host "RUN: $Run"
    Write-Host "REPORT: $(Join-Path $Analysis 'atlas4_analysis.json')"
